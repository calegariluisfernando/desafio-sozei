...
Vagrant.configure("2") do |config|
  ...
  config.vm.provider "virtualbox" do |vb|
    ...
    # vb.memory = "1024"
    vb.name = "sozei-dev"
    vb.customize ["modifyvm", :id, "--groups", "/Sozei"]
  end
  #
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    if test -f "$MYSQLDCONF"; then
      ...
      mysql -uroot -ptoor -e \
        "CREATE DATABASE desafioSozei; CREATE DATABASE desafioSozeiTest; CREATE USER 'root'@'%' IDENTIFIED BY 'toor'; GRANT ALL PRIVILEGES ON *.* TO 'root'@'%'; ALTER USER 'root'@'%' IDENTIFIED WITH mysql_native_password BY 'toor'"
